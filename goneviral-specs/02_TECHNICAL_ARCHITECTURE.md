...
| Database/Auth/Storage | Supabase Free, Mumbai                      | PostgreSQL, passwordless Auth and Storage with accepted Free-plan recovery risk |
...
- platform metrics and GoneViral's encrypted logical/Storage backups support
  operations; managed backups/PITR are not assumed on Free.
...
GitHub Actions --------> authenticated scheduled route calls
```
...
Financial transaction inserts outbox row atomically. After commit, an
authenticated GitHub Actions schedule invokes the Vercel function that drains
rows with idempotent provider message keys, bounded retries and status tracking.
...
| production      | Vercel Pro project         | production Free project    | live     | production domain   | goneviral.in              |
...
Rejected for a live commercial money product under Vercel's current terms. Use
for development/private testing only. Supabase Free is separately accepted by
the owner with no managed PITR, possible inactivity pause, self-managed backup
dependence, and possible recovery downtime; see
